...
void ocolos_env::read_config() {
    ...
    std::string config_s = dir_path_str + "config";
    ...
    while (fgets(line, 4096, config) != NULL) {
        ...
        if ((current_line.length() > 0) && (current_line[0] != '#')) {
            ...
            std::string bin_name = current_line.substr(0, current_line.find(delimiter));
            std::string path_name = current_line.substr(current_line.find(delimiter), current_line.length());
...
void ocolos_env::get_cmds() {
    ...
    std::vector<std::string> run_benchmark_cmd_args = split_line(ocolos_env::run_benchmark_cmd);

    bool has_db = false;
    bool has_user = false;
    for (unsigned int i = 0; i < run_benchmark_cmd_args.size(); i++) {
        if (run_benchmark_cmd_args[i].length() > 10) {
            std::string first11 = run_benchmark_cmd_args[i].substr(0, 11);
            if (first11 == "--mysql-db=") {
        ...
        if (run_benchmark_cmd_args[i].length() > 12) {
            std::string first13 = run_benchmark_cmd_args[i].substr(0, 13);
