...
int main() {
    ...
    if (target_pid == 0) {
        ...
        std::thread t = std::thread(send_data_path, &ocolos_environ);
        ...
        std::unordered_map<long, func_info> bolted_func = run_llvmbolt(&ocolos_environ);
        ...
        process_vtables(ocolos_environ.bolted_binary_path.c_str(), std::to_string(target_pid),
                        ocolos_environ.v_table_bin.c_str());
        ...
        std::vector<pid_t> tids = pause_and_get_tids(target_pid);
