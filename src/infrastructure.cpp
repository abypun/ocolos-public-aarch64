...
void initialize_benchmark(const ocolos_env *ocolos_environ) {
    sleep(20);
    std::string command = ocolos_environ->client_binary_path + " -u " + ocolos_environ->db_user_name +
                     " -e 'drop database " + ocolos_environ->db_name + "'";
    if (system(command.c_str()))
        exit(-1);
    command = ocolos_environ->client_binary_path + " -u " + ocolos_environ->db_user_name + " -e 'create database " +
    ...
    if (sysbench_init_pid == 0) {
        // child
        std::string benchmark_cmd = ocolos_environ->init_benchmark_cmd;
...
void run_benchmark(const ocolos_env *ocolos_environ) {
    ...
    if (sysbench_pid == 0) {
        ...
        dup2(fd, 1);
        std::string benchmark_cmd = ocolos_environ->run_benchmark_cmd;
        char **argv = split_str_2_char_array(benchmark_cmd);
        char **envp = environ;
        execve(argv[0], argv, envp);
    }
...
void create_target_server_process(const ocolos_env *ocolos_environ) {
    std::string ld_pre = "LD_PRELOAD=" + ocolos_environ->ld_preload_path;
    char *ldpreload = new char[ld_pre.length() + 1];
    strcpy(ldpreload, ld_pre.c_str());

    std::string exe_cmd = ocolos_environ->run_server_cmd;
...
void send_data_path(const ocolos_env *ocolos_environ) {
    ...
    std::string path = ocolos_environ->tmp_data_path;
...
void run_perf_record(int target_pid, const ocolos_env *ocolos_environ) {
    ...
    if (func_exec_count == 1) {
        command = ocolos_environ->perf_path + " record -e cycles:u -j any,u -a -o " + ocolos_environ->tmp_data_path +
                  "perf.data -p " + to_string(target_pid) + " -- sleep 60";
    } else {
        stringstream ss;
        ss << func_exec_count;
        std::string f_exec_count = ss.str();
        command = ocolos_environ->perf_path + " record -e cycles:u -j any,u -a -o " + ocolos_environ->tmp_data_path +
                  "perf" + f_exec_count + ".data -p " + to_string(target_pid) + " -- sleep 60";
    }
#endif
#ifdef AArch64
    if (func_exec_count == 1) {
        command = ocolos_environ->perf_path + " record -e cycles:u  -a -o " + ocolos_environ->tmp_data_path +
                  "perf.data -p " + std::to_string(target_pid) + " -- sleep 60";
    } else {
        std::stringstream ss;
        ss << func_exec_count;
        std::string f_exec_count = ss.str();
        command = ocolos_environ->perf_path + " record -e cycles:u  -a -o " + ocolos_environ->tmp_data_path + "perf" +
                  f_exec_count + ".data -p " + std::to_string(target_pid) + " -- sleep 60";
    }
#endif
    char *command_cstr = new char[command.length() + 1];
    strcpy(command_cstr, command.c_str());
    if (system(command_cstr) != 0)
...
void run_perf2bolt(const ocolos_env *ocolos_environ) {
    ...
    if (func_exec_count == 1) {
#ifdef Intel64
        std::string command = "" + ocolos_environ->perf2bolt_path + " -p " + ocolos_environ->tmp_data_path +
                         "perf.data -o " + ocolos_environ->tmp_data_path + "perf.fdata " +
...
#endif
        std::vector<std::string> BOLTed_addr;
        ...
        if (has_sharp) {
            std::string file_path = ocolos_environ->tmp_data_path + "BOLTed_bin_info.txt";
        ...
        std::string command = "" + ocolos_environ->perf2bolt_path + " --ignore-build-id --cont-opt" + " -p " +
                         ocolos_environ->tmp_data_path + "perf" + f_exec_count + ".data -o " +
                         ocolos_environ->tmp_data_path + "perf" + f_exec_count + ".fdata " +
                         ocolos_environ->target_binary_path;
        char *command_cstr = new char[command.length() + 1];
        strcpy(command_cstr, command.c_str());
        ...
        while (fgets(path1, sizeof(path1), fp1) != NULL) {
            ...
            std::string file_path = ocolos_environ->dir_path + "BOLTed_bin_info.txt";
            FILE *fp = fopen(file_path.c_str(), "w");
            fprintf(fp, "%s\n", ocolos_environ->bolted_binary_path.c_str());
...
std::unordered_map<long, func_info> run_llvmbolt(const ocolos_env *ocolos_environ) {
#ifdef TIME_MEASUREMENT
    auto begin = std::chrono::high_resolution_clock::now();
#endif
    ...
    char path1[3000];
#ifdef Intel64
    std::string command = "" + ocolos_environ->llvmbolt_path + " " + ocolos_environ->target_binary_path + " -b " +
                     ocolos_environ->tmp_data_path + "perf.fdata -o " + ocolos_environ->bolted_binary_path +
                     " --enable-bat" + " -reorder-blocks=ext-tsp " + "-reorder-functions=hfsort+ " +
                     "-split-functions=0 " + "-dyno-stats";
    ...
    std::string command = "" + ocolos_environ->llvmbolt_path + " " + ocolos_environ->target_binary_path + " -b " +
                     ocolos_environ->tmp_data_path + "perf.fdata -o " + ocolos_environ->bolted_binary_path +
                     " --enable-bat --enable-aobo " + " -reorder-blocks=ext-tsp " + "-reorder-functions=hfsort+ " +
                     "-split-functions=0 " + "-dyno-stats";
